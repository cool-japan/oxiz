...
fn test_external_branching_field_accepts_arc_mutex() {
    ...
    config.external_branching = Some(heuristic);
...
fn test_external_branching_called_during_solve() {
    ...
    let mut config = SolverConfig::default();
    config.external_branching = Some(heuristic.clone());
...
fn test_external_branching_deferring_still_solves() {
    ...
    let mut config = SolverConfig::default();
    config.external_branching = Some(Arc::new(Mutex::new(DeferringHeuristic)));
...
fn test_external_branching_unsat_formula() {
    ...
    let mut config = SolverConfig::default();
    config.external_branching = Some(Arc::new(Mutex::new(CountingHeuristic::new())));
...
fn test_external_branching_highest_score_heuristic_solves() {
    ...
    let mut config = SolverConfig::default();
    config.external_branching = Some(Arc::new(Mutex::new(HighestScoreHeuristic)));

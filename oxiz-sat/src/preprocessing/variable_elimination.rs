...
use crate::{Clause, Lit, Solver, Var};
...
impl VariableEliminator {
    ...
    pub fn eliminate(&mut self, clauses: &mut Vec<Clause>) -> Vec<Var> {
        ...
        while let Some(var) = queue.pop_front() {
            if self.should_eliminate(var, clauses, &occurrence_map) {
                if self.eliminate_variable(var, clauses, &mut occurrence_map) {
                    eliminated.push(var);
    ...
    fn should_eliminate(
            ...
            .flatten()
        {
            if let Some(clause) = clauses.get(idx) {
                if clause.lits.len() > self.config.max_clause_size {
                    return false;
                }
            }
        }
    ...
    fn eliminate_variable(
        ...
        let to_remove: HashSet<usize> = pos_clauses
            .into_iter()
            .chain(neg_clauses.into_iter())
            .collect();

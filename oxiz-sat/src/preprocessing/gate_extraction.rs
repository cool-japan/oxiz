...
impl GateExtractor {
    ...
    fn extract_gate_for_var(
        ...
        if self.config.extract_and {
            if let Some(gate) = self.try_extract_and(var, clauses, defining_clauses, occurrence_map)
            {
        ...
        if self.config.extract_or {
            if let Some(gate) = self.try_extract_or(var, clauses, defining_clauses, occurrence_map)
            {
        ...
        if self.config.extract_xor {
            if let Some(gate) = self.try_extract_xor(var, clauses, defining_clauses, occurrence_map)
            {
        ...
        if self.config.extract_ite {
            if let Some(gate) = self.try_extract_ite(var, clauses, defining_clauses, occurrence_map)
            {

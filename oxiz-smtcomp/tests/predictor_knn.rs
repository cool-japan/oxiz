...
use oxiz_smtcomp::predictor::{Dataset, DifficultyModel, Features, KnnRegressor, Sample, TrainingConfig};
...
fn test_knn_predict_matches_nearest_neighbour_on_k1() {
    ...
    let q_a = Features { atom_count: 0.5, ..Default::default() };
    let rt_a = model.predict_runtime(&q_a);
    assert!(rt_a < 1.0, "Expected trivial runtime near cluster A, got {rt_a}");

    // Query near cluster B → should predict large runtime
    let q_b = Features { atom_count: 100.5, ..Default::default() };
    let rt_b = model.predict_runtime(&q_b);
    assert!(rt_b > 1.0, "Expected hard runtime near cluster B, got {rt_b}");
}
...
fn test_knn_inverse_distance_weighting() {
    ...
    let q = Features { atom_count: 1.0, ..Default::default() };
...
fn test_knn_handles_zero_distance_without_nan() {
    ...
    let q = Features { atom_count: 42.0, ..Default::default() };
    let rt = model.predict_runtime(&q);

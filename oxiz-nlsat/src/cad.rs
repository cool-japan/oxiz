...
impl CadProjection {
    ...
    fn compute_resultant_cached(&self, p1: &Polynomial, p2: &Polynomial, var: Var) -> Polynomial {
        ...
        {
            let cache = self.resultant_cache.read().expect("lock should not be poisoned");
        ...
        {
            let mut cache = self.resultant_cache.write().expect("lock should not be poisoned");
...
impl CadLifter {
    ...
    pub fn lift(&self, assignment: &FxHashMap<Var, BigRational>, var_idx: usize) -> Vec<CadPoint> {
        ...
        if all_roots.is_empty() {
            ...
            // After last root
            let after = all_roots.last().expect("collection validated to be non-empty") + BigRational::one();
...
impl CadDecomposer {
    ...
    fn decompose_base(&mut self) -> Result<Vec<CadCell>, CadError> {
        ...
        if all_roots.is_empty() {
            ...
            let last = all_roots.last().expect("collection validated to be non-empty");
    ...
    fn lift_single_cell(&self, cell: &CadCell, var: Var, level: usize) -> Vec<CadCell> {
        ...
        if cell_roots.is_empty() {
            ...
            // After last root
            let sample = self.select_sample_point(Some(cell_roots.last().expect("collection validated to be non-empty")), None, var);
            let mut new_sample = cell.sample.clone();
            new_sample.push(CadPoint::rational(sample));
            let mut new_var_order = cell.var_order.clone();
            new_var_order.push(var);

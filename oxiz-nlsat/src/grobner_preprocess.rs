...
mod tests {
    ...
    fn test_skip_complex_system() {
        let mut config = GroebnerConfig::default();
        config.max_poly_count = 2; // Very restrictive limit
    ...
    fn test_stats_tracking() {
        ...
        let p1 = poly_from_coeffs(0, &[1, 1]);
        let _result = preprocessor.preprocess(&[p1.clone()]);
    ...
    fn test_degree_reduction() {
        ...
        match result {
            PreprocessResult::Simplified { polynomials, .. } => {

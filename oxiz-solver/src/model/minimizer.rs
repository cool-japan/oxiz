...
impl ModelMinimizer {
    ...
    pub fn minimize(&mut self) -> Vec<Assignment> {
        ...
        for _ in 0..self.config.max_iterations {
            ...
            if self.config.remove_unconstrained {
                if self.try_remove_assignments(&mut minimized) {
                    changed = true;
                }
            }

            // Try to minimize values
            if self.config.minimize_values {
                if self.try_minimize_values(&mut minimized) {
                    changed = true;
                }
            }

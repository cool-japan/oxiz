...
use oxiz_core::ast::term::{TermId, TermKind};
...
impl SkolemizationContext {
    ...
    pub fn skolemize(
        ...
        let nnf = self.to_nnf(tm, term, false)?;
    ...
    fn to_nnf(
        ...
        let result = match kind {
            ...
            TermKind::Not(arg) => {
                ...
                self.to_nnf(tm, arg, !negated)
            }
            TermKind::And(args) => {
                let mut nnf_args = Vec::with_capacity(args.len());
                for &a in &args {
                    nnf_args.push(self.to_nnf(tm, a, negated)?);
                }
                if negated {
            ...
            TermKind::Implies(lhs, rhs) => {
                // (a -> b) = (NOT a) OR b
                // NOT(a -> b) = a AND (NOT b)
                let lhs_nnf = self.to_nnf(tm, lhs, !negated)?;
                let rhs_nnf = self.to_nnf(tm, rhs, negated)?;
                if negated {
            ...
            TermKind::Xor(lhs, rhs) => {
                ...
                let a = self.to_nnf(tm, lhs, false)?;
                let b = self.to_nnf(tm, rhs, false)?;
                let not_a = self.to_nnf(tm, lhs, true)?;
                let not_b = self.to_nnf(tm, rhs, true)?;
            ...
            TermKind::Forall {
                ...
                let body_nnf = self.to_nnf(tm, body, negated)?;
                ...
                if negated {
                    ...
                    Ok(tm.mk_exists_with_patterns(
                        var_names.iter().map(|(s, sort): &(String, SortId)| (s.as_str(), *sort)),
                        body_nnf,
                        patterns,
                    ))
                } else {

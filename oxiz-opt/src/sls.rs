...
impl SlsSolver {
    ...
    pub fn solve(&mut self) -> Result<SlsResult, SlsError> {
        ...
        for iter in 0..self.config.max_iterations {
            ...
            // Check if all hard clauses are satisfied
            if !self.cache.as_ref().expect("cache initialized at solve start").all_hard_satisfied() {
    ...
    fn update_cache(&mut self) {
        let assignment = self.assignment.as_ref().expect("assignment initialized at solve start");
    ...
    fn compute_cost(&self) -> Weight {
        let cache = self.cache.as_ref().expect("cache initialized at solve start");

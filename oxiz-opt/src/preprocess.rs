...
impl Preprocessor {
    ...
    fn bounded_variable_elimination(&mut self, soft_clauses: Vec<SoftClause>) -> Vec<SoftClause> {
        ...
        for var in vars_to_eliminate {
            let pos_indices = var_pos_clauses.get(&var).expect("variable exists in pos_clauses map");
            let neg_indices = var_neg_clauses.get(&var).expect("variable exists in neg_clauses map");

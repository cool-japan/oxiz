...
impl RegexSolver {
    ...
    pub fn test_membership(&mut self, string: &str, regex: &Regex) -> Result<bool, String> {
        ...
        match regex {
            ...
            Regex::CharClass(chars) => Ok(
                string.len() == 1 && string.chars().next().map_or(false, |c| chars.contains(&c))
            ),
    ...
    fn test_concat_recursive(
        ...
        for split_pos in 0..=string.len() {
            ...
            if self.test_membership(prefix, &parts[part_idx])? {
                if self.test_concat_recursive(suffix, parts, part_idx + 1)? {
                    return Ok(true);
                }
            }
        }
    ...
    fn test_star(&mut self, string: &str, inner: &Regex) -> Result<bool, String> {
        ...
        for end in 1..=string.len() {
            ...
            if self.test_membership(prefix, inner)? {
                if self.test_star(suffix, inner)? {
                    return Ok(true);
                }
            }
        }

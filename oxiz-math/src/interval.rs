...
impl Interval {
    ...
    pub fn mul(&self, other: &Interval) -> Interval {
        ...
        let (lo, lo_open) = bounds.first().expect("collection validated to be non-empty").clone();
        let (hi, hi_open) = bounds.last().expect("collection validated to be non-empty").clone();

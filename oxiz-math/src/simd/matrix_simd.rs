    ...
    T: Clone + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + Float,
{
    ...
    for k in 0..n {
        ...
        for j in k..n {
            ...
            for s in 0..k {
                u[k][j] = u[k][j].clone() - l[k][s].clone() * u[s][j].clone();
            }
        }
        ...
        for i in (k + 1)..n {
            ...
            for s in 0..k {
                l[i][k] = l[i][k].clone() - l[i][s].clone() * u[s][k].clone();
            }

            l[i][k] = l[i][k].clone() / u[k][k].clone();
        }
    }
    ...
    T: Clone + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + Float,
{
    ...
    for i in 0..n {
        b_perm[i] = b[perm[i]].clone();
    }

    // Forward substitution (L * y = b_perm)
    let mut y = vec![T::zero(); n];
    for i in 0..n {
        let mut sum = b_perm[i].clone();
        for j in 0..i {
    ...
    T: Clone + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + Float,
{
    ...
    for j in 0..cols {
        // Get column j
        let mut v: Vec<T> = matrix.iter().map(|row| row[j].clone()).collect();

        // Orthogonalize against previous columns
        for i in 0..j {
            let q_col: Vec<T> = q.iter().map(|row| row[i].clone()).collect();

            let dot = dot_product(&q_col, &v);
            r[i][j] = dot.clone();

            for k in 0..rows {
                v[k] = v[k].clone() - q_col[k].clone() * dot.clone();
            }
        }

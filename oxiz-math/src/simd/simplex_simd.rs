    ...
    T: Clone + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + Div<Output = T> + Float,
{
    ...
    fn find_leaving_variable(&self, entering: usize) -> Result<Option<usize>, SimplexError> {
        ...
        for i in 0..self.num_constraints {
            let coeff = self.tableau[i][entering].clone();

            if coeff > T::epsilon() {
                let rhs = self.tableau[i][rhs_col].clone();
                let ratio = rhs / coeff;
    ...
    fn perform_pivot(&mut self, entering: usize, leaving: usize) {
        ...
        // Get pivot element
        let pivot = self.tableau[leaving][entering].clone();
        ...
        // Normalize pivot row
        for j in 0..num_cols {
            self.tableau[leaving][j] = self.tableau[leaving][j].clone() / pivot.clone();
        }
        ...
        for i in 0..=self.num_constraints {
            ...
            let factor = self.tableau[i][entering].clone();
            ...
            for chunk_start in (0..num_cols).step_by(CHUNK_SIZE) {
                ...
                for j in chunk_start..chunk_end {
                    let update = self.tableau[leaving][j].clone() * factor.clone();
                    self.tableau[i][j] = self.tableau[i][j].clone() - update;
                }
            }
        }
    }
    ...
    pub fn get_solution(&self) -> Vec<T> {
        ...
        for (row, &var_idx) in self.basis.iter().enumerate() {
            if var_idx < self.num_vars {
                solution[var_idx] = self.tableau[row][rhs_col].clone();
            }
        }
    ...
    pub fn get_objective_value(&self) -> T {
        let total_vars = self.num_vars + self.num_constraints;
        let rhs_col = total_vars;
        self.tableau[self.num_constraints][rhs_col].clone()
    }
    ...
    T: Clone + Float,
{
    ...
    for i in 0..tableau.num_constraints {
        let rhs = tableau.tableau[i][rhs_col].clone();
        if rhs < min_val {
            min_val = rhs;
            min_idx = Some(i);
        }
    ...
    T: Clone + Float + Div<Output = T>,
{
    ...
    for j in 0..total_vars {
        let coeff = leaving_row[j].clone();

        if coeff < T::zero() - T::epsilon() {
            let obj_coeff = obj_row[j].clone();
